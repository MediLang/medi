...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut entries: Vec<_> = fs::read_dir(&base)?
        ...
        .collect();
    entries.sort();
    let csv = entries.last().ok_or("No CSV found; run benchmark_parser_scaling first")?.clone();
    ...
    let out_svg = base.join("scaling_plot.svg");
    {
        ...
        let y_max = data.iter().map(|r| r.time_ms).fold(0.0/0.0, f64::max).max(1.0);
        let mut chart = ChartBuilder::on(&root)
            .caption("Parser scaling (avg over 3 runs)", ("sans-serif", 20).into_font())
            .margin(10)
        ...
        for (i, kind) in kinds.iter().enumerate() {
            let mut pts: Vec<(u64, f64)> = data.iter().filter(|r| &r.kind == kind).map(|r| (r.bytes, r.time_ms)).collect();
            pts.sort_by_key(|p| p.0);
            let color = Palette99::pick(i).mix(1.0);
            chart.draw_series(LineSeries::new(pts.clone(), &color))?
                .label(kind.as_str())
    ...
    let out_svg_log = base.join("scaling_plot_loglog.svg");
    {
        ...
        chart.configure_mesh()
            ...
            .draw()?;
        for (i, kind) in kinds.iter().enumerate() {
            let mut pts: Vec<(f64, f64)> = data.iter().filter(|r| &r.kind == kind)
                .map(|r| ((r.bytes as f64).ln_1p(), (r.time_ms).ln_1p())).collect();
