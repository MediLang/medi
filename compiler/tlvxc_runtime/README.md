...
- Macro `tolvex_runtime_report!(err, "op_tag")` captures source context automatically (file/line/column/module/op) and reports via context-aware reporter when set, or falls back to minimal.
...
- `RtRegion<const BYTES: usize>` bump allocator with `alloc_*` and `alloc_*_result` variants.
  - `*_result` variants report overflow with context via `tolvex_runtime_report!` and return `Result`.
