...
fn test_nested_member_expressions() {
    ...
    if let ExpressionNode::Identifier(ident) = base_expr {
        ...
        panic!("Expected identifier 'patient' at the start of member chain, got {:?}", base_expr);
    }
    ...
    println!("\nTokens for 'hospital.patient_id.records.blood_pressure': {:?}", tokens);
    ...
    if let ExpressionNode::Identifier(ident) = inner_expr {
        ...
        panic!("Expected identifier 'hospital' at the start of member chain, got {:?}", inner_expr);
    }
    
    // Test with 'Patient' as a keyword (case-sensitive)
    let tokens = tokenize("hospital.Patient.records.blood_pressure");
    println!("\nTokens for 'hospital.Patient.records.blood_pressure': {:?}", tokens);
