...
fn realdata_patients_observations_events_optional() {
    let patients_path = match load_env("TOLVEX_DATA_PATIENTS") {
        Some(p) => p,
        None => return,
    };
    let observations_path = match load_env("TOLVEX_DATA_OBSERVATIONS") {
        Some(p) => p,
        None => return,
    };
    let events_path = match load_env("TOLVEX_DATA_EVENTS") {
        Some(p) => p,
        None => return,
    };
